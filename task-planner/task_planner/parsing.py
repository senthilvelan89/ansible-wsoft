"""Parsing and formatting helpers for dates."""
...
def parse_date(value, reference: Optional[dt.date] = None) -> dt.date:
    ...
    Supported: empty/``today``/``now``, ``yesterday``, ``tomorrow``,
    ISO ``YYYY-MM-DD``, ``YYYY/MM/DD``, ``MM-DD`` (nearest past occurrence)
    and ``-N`` / ``+N`` for relative days.
    """
...
def parse_optional_date(value, reference: Optional[dt.date] = None) -> Optional[dt.date]:
    """Like ``parse_date`` but blank / None stays unset."""
...
def format_date(value: Optional[dt.date]) -> Optional[str]:
    if value is None:
        return None
